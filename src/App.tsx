    ...
    <div className="min-h-screen bg-slate-50 font-sans selection:bg-indigo-100 selection:text-indigo-900">
      <header className="bg-white/80 backdrop-blur-md border-b border-slate-200/60 sticky top-0 z-50">
        <div className="mx-auto max-w-7xl px-4 md:px-8 h-20 flex items-center justify-between">
          <div className="flex items-center gap-4 group cursor-pointer">
            <div className="bg-indigo-600 p-2.5 rounded-2xl shadow-lg shadow-indigo-200 group-hover:scale-110 group-hover:rotate-3 transition-all duration-300">
              <Truck className="h-6 w-6 text-white" />
            </div>
            <div>
              <h1 className="text-xl font-extrabold tracking-tight text-slate-900 font-heading leading-none">Amanda Vasconcelos</h1>
              <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mt-1 hidden sm:block">Inteligência Logística</p>
            </div>
          </div>
      ...
      <main className="mx-auto max-w-7xl p-4 md:p-8 space-y-8">
        ...
        <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
          <TabsList className="mb-8 bg-slate-100/50 p-1.5 rounded-2xl border border-slate-200/60 h-auto gap-1">
            <TabsTrigger value="audit" className="flex items-center gap-2 rounded-xl px-6 py-2.5 text-sm font-bold transition-all data-[state=active]:bg-white data-[state=active]:text-indigo-600 data-[state=active]:shadow-sm border border-transparent data-[state=active]:border-slate-200/60">
              <FileSpreadsheet className="h-4 w-4" /> Auditoria
            </TabsTrigger>
            <TabsTrigger value="help-center" className="flex items-center gap-2 rounded-xl px-6 py-2.5 text-sm font-bold transition-all data-[state=active]:bg-white data-[state=active]:text-indigo-600 data-[state=active]:shadow-sm border border-transparent data-[state=active]:border-slate-200/60">
              <MessageSquare className="h-4 w-4" /> Suporte Técnico
            </TabsTrigger>
            <TabsTrigger value="changelog" className="flex items-center gap-2 rounded-xl px-6 py-2.5 text-sm font-bold transition-all data-[state=active]:bg-white data-[state=active]:text-indigo-600 data-[state=active]:shadow-sm border border-transparent data-[state=active]:border-slate-200/60">
          ...
          <TabsContent value="audit" className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 bg-white p-6 rounded-2xl border border-slate-200/60 shadow-sm">
              <div>
                <h2 className="text-2xl font-bold font-heading text-slate-900 tracking-tight">Nova Auditoria</h2>
                <p className="text-sm text-slate-500 font-medium">Carregue os arquivos para iniciar o batimento financeiro.</p>
              </div>
              <div className="flex flex-wrap gap-2">
                <Button 
                  ...
                  className="rounded-xl border-slate-200 text-slate-600 hover:bg-slate-50 hover:text-rose-600 hover:border-rose-200"
                >
                  <RefreshCcw className="mr-2 h-4 w-4" /> Limpar
                </Button>
                {results.length > 0 && (
                  <div className="flex gap-2">
                    <Button onClick={() => exportToExcel(results)} variant="outline" className="rounded-xl border-emerald-200 text-emerald-700 hover:bg-emerald-50">
                      <FileSpreadsheet className="mr-2 h-4 w-4" /> Excel
                    </Button>
                    <Button onClick={() => exportToPDF(results, summary)} variant="outline" className="rounded-xl border-rose-200 text-rose-700 hover:bg-rose-50">
                      <FileText className="mr-2 h-4 w-4" /> PDF
                    </Button>
                    <Button onClick={() => shareToWhatsApp(results, summary)} className="rounded-xl bg-emerald-600 hover:bg-emerald-700 text-white shadow-lg shadow-emerald-100">
                  ...
                  {!isParsingA && fileA && columnsA.length === 0 && (
                    <div className="p-4 bg-amber-50 text-amber-700 rounded-lg border border-amber-200 text-sm flex flex-col gap-2">
                      <div className="flex items-center gap-2 font-semibold">
                        ...
                        <span>Nenhum dado identificado no Relatório ATUA.</span>
                      </div>
                ...
                <CardContent className="space-y-4 pt-6">
                  ...
                  {!isParsingB && fileB && columnsB.length === 0 && (
                    <div className="p-4 bg-amber-50 text-amber-700 rounded-lg border border-amber-200 text-sm flex flex-col gap-2">
                      <div className="flex items-center gap-2 font-semibold">
                        <RefreshCcw className="h-4 w-4" />
                        <span>Nenhum dado identificado no Relatório GW.</span>
                      </div>
                      <p className="text-xs opacity-90">
                        O sistema não conseguiu extrair tabelas deste arquivo. Verifique se o PDF possui texto selecionável (não é apenas uma foto/scan) ou tente outro formato.
                      </p>
      ...
      <footer className="mx-auto max-w-7xl px-4 md:px-8 py-8 border-t border-zinc-200 mt-12">
        <div className="flex flex-col md:flex-row justify-between items-center gap-4">
          <p className="text-sm text-zinc-500">© {new Date().getFullYear()} Amanda Vasconcelos Logística. Todos os direitos reservados.</p>
