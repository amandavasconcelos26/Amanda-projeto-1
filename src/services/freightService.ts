...
// Configurar o worker do PDF.js usando CDN para evitar problemas de empacotamento
  ...
  return new Promise(async (resolve, reject) => {
    ...
    if (extension === 'csv') {
      ...
      if (errorStr.includes("429") || errorStr.includes("quota")) {
        ...
        userMsg = "Chave de API inválida ou ausente. Verifique a configuração da GEMINI_API_KEY.";
      } else {
        userMsg = `Falha ao extrair dados do arquivo: ${error.message || errorStr}`;
      }
...
export const exportToExcel = (results: AuditResult[]) => {
  const exportData = results.map(r => ({
    'CTE': r.cte,
    'Status': r.status === 'A_ONLY' ? 'Apenas Relatório ATUA' : 
...
export const shareToWhatsApp = (results: AuditResult[], summary: AuditSummary) => {
  ...
  if (issues.length > 0) {
    ...
    issues.slice(0, 10).forEach(r => {
      ...
      else if (r.status === 'B_ONLY') text += `- CTE ${r.cte}: Faltante no Relatório ATUA\n`;
